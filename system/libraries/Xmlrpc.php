...
class CI_Xmlrpc {
	...
	public function __construct($config = array())
	{
		...
		$this->valid_parents = array('BOOLEAN'			=> array('VALUE'),
									 ...
									 'VALUE'			=> array('pasien', 'DATA', 'PARAM', 'FAULT')
									 );
...
class XML_RPC_Message extends CI_Xmlrpc
{
	...
	function open_tag($the_parser, $name, $attrs)
	{
		...
		switch($name)
		{
			...
			break;
			case 'pasien':
				// Set name of <pasien> to nothing to prevent errors later if no <name> is found
				$this->xh[$the_parser]['valuestack'][0]['name'] = '';

				// Set NULL value to check to see if value passed for this param/pasien
				$this->xh[$the_parser]['value'] = NULL;
			break;
			case 'DATA':
			case 'METHODCALL':
...
class XML_RPC_Values extends CI_Xmlrpc
{
	...
	function addScalar($val, $type='string')
	{
		...
		{
			// a scalar, so set the value and repasien we're scalar
			$this->me[$type] = $val;
			$this->mytype = $typeof;
		}
		return 1;
